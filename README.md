...
5. **Уведомления** - отправка статуса в Telegram
...
```
plex-audio-converter/
├── audio_converter.py      # Основной скрипт конвертации
├── audio_monitor.py         # Скрипт автоматического мониторинга
├── monitor_config.ini       # Конфигурация мониторинга (создается автоматически)
├── requirements.txt         # Python зависимости
├── .audio_converter_state.json  # Технический файл состояния (в каждой папке)
├── processed_files.json    # История обработанных файлов
└── audio_converter.log      # Лог файл
```
